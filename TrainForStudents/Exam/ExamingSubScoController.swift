...
class ExamingSubScoController: UIViewController,UITableViewDelegate,UITableViewDataSource,SubScoreCellDelegate {
    ...
    @objc func sureBtnTapped() {
        ...
        let parentInfo = (self.parent as! StillExamController).headInfo
    ...
    func requestQuestionItem(questionsid:String,isNextQuestionBtnTapped:Bool) {
        ...
        let urlString = SERVER_PORT + "rest/skillQuestion/querySkillquestionsitem.do"
        myPostRequest(urlString, ["questionsid":questionsid], method: HTTPMethod.post).responseJSON { (response) in
            MBProgressHUD.hide(for: self.view, animated: true)
            switch(response.result){
            case .failure(let error):
                print(error)
