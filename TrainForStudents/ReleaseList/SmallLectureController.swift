...
class SmallLectureController : MyBaseUIViewController{
    ...
    @IBOutlet weak var files_view: UIView!
    ...
    override func viewDidLoad() {
        ...
        buttonGroup = [btn_baseInfo ,btn_students ,btn_files]
        
        students_view.frame.origin = CGPoint(x: UIScreen.width, y: students_view.frame.origin.y)
        files_view.frame.origin = CGPoint(x: UIScreen.width.multiplied(by: 2), y: files_view.frame.origin.y)
        ...
        //附件设置
        let filesCollection = FilesCollectionView(rect: CGRect(x: 0, y: 0, width: UIScreen.width, height: files_view.frame.height))

        files_view.addSubview(filesCollection)
    ...
    func tabsTouchAnimation( sender : UIButton){
        ...
        }else if sender.restorationIdentifier == "btn_files"{
...
extension SmallLectureController : UIScrollViewDelegate{
    ...
    func scrollViewWillEndDragging(_ scrollView: UIScrollView, withVelocity velocity: CGPoint, targetContentOffset: UnsafeMutablePointer<CGPoint>) {
        ...
        if x < UIScreen.width.divided(by: 2){
            ...
            tabsTouchAnimation(sender: btn_files)
        }
    }
